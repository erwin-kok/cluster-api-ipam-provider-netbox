...
// NetboxPrefixPoolReconciler reconciles a NetboxPrefixPool object
type NetboxPrefixPoolReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

func AddNetboxPrefixPoolReconciler(mgr manager.Manager) error {
	reconciler := &NetboxPrefixPoolReconciler{
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			...
			handler.EnqueueRequestsFromMapFunc(func(_ context.Context, clientObj client.Object) []reconcile.Request {
				...
				return ipAddressToNetboxIPPool(ipAddress, ipamv1alpha1.NetboxPrefixPoolKind)
			}),
		).
		Complete(reconciler)
}
...
func (r *NetboxPrefixPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	logger := log.FromContext(ctx)
	logger.Info("Reconciling NetboxPrefixPool")

	pool := &ipamv1alpha1.NetboxPrefixPool{}
