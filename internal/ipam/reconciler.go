...
func AddIPAddressClaimReconciler(ctx context.Context, mgr manager.Manager, watchFilter string) error {
	...
	reconciler := &IPAddressClaimReconciler{
		Client:           mgr.GetClient(),
	...
	b := ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			&clusterv1.Cluster{},
			ctrlhandler.EnqueueRequestsFromMapFunc(reconciler.clusterToIPClaims),
	...
	if err := reconciler.Adapter.SetupWithManager(ctx, b); err != nil {
		return err
	}
...
func (r *IPAddressClaimReconciler) reconcileDelete(ctx context.Context, claim *ipamv1.IPAddressClaim) error {
